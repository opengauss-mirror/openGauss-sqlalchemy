...
@create_db.for_db("opengauss")
def _opengauss_create_db(cfg, eng, ident):
    template_db = cfg.options.opengauss_templatedb

    with eng.execution_options(isolation_level="AUTOCOMMIT").begin() as conn:
        try:
            _opengauss_drop_db(cfg, conn, ident)
        except Exception:
            pass
        if not template_db:
...
@drop_db.for_db("opengauss")
def _opengauss_drop_db(cfg, eng, ident):
...
@temp_table_keyword_args.for_db("opengauss")
def _opengauss_temp_table_keyword_args(cfg, eng):
    return {"prefixes": ["TEMPORARY"]}


@set_default_schema_on_connection.for_db("opengauss")
def _opengauss_set_default_schema_on_connection(
...
@drop_all_schema_objects_pre_tables.for_db("opengauss")
...
@drop_all_schema_objects_post_tables.for_db("opengauss")
...
@prepare_for_drop_tables.for_db("opengauss")
def prepare_for_drop_tables(config, connection):
    ...
    if rows:
        warn_test_suite(
            "OpenGauss may not be able to DROP tables due to "
